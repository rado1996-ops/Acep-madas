 ...
 * @author Chris Corbyn
 */
class Swift_Mime_SimpleMimeEntity implements \MailPoetVendor\Swift_Mime_MimeEntity
{
     ...
     */
    public function __construct(\MailPoetVendor\Swift_Mime_HeaderSet $headers, \MailPoetVendor\Swift_Mime_ContentEncoder $encoder, \MailPoetVendor\Swift_KeyCache $cache, \MailPoetVendor\Swift_Mime_Grammar $grammar)
    {
        $this->_cacheKey = \md5(\uniqid(\getmypid() . \mt_rand(), \true));
        $this->_cache = $cache;
    ...
    {
        return $this->_body instanceof \MailPoetVendor\Swift_OutputByteStream ? $this->_readStream($this->_body) : $this->_body;
    }
    /**
    ...
    public function getBoundary()
    {
        if (!isset($this->_boundary)) {
            $this->_boundary = '_=_swift_v4_' . \time() . '_' . \md5(\getmypid() . \mt_rand() . \uniqid('', \true)) . '_=_';
        }
    ...
    protected function _bodyToString()
    {
        ...
        if (isset($this->_body) && empty($this->_immediateChildren)) {
            if ($this->_cache->hasKey($this->_cacheKey, 'body')) {
                ...
                $this->_cache->setString($this->_cacheKey, 'body', $body, \MailPoetVendor\Swift_KeyCache::MODE_WRITE);
            }
            $string .= $body;
        }
     ...
     * @param Swift_InputByteStream
     */
    protected function _bodyToByteStream(\MailPoetVendor\Swift_InputByteStream $is)
    {
        if (empty($this->_immediateChildren)) {
            if (isset($this->_body)) {
                if ($this->_cache->hasKey($this->_cacheKey, 'body')) {
                    ...
                    $is->write("\r\n");
                    if ($this->_body instanceof \MailPoetVendor\Swift_OutputByteStream) {
    ...
    {
        $idLeft = \md5(\getmypid() . '.' . \time() . '.' . \uniqid(\mt_rand(), \true));
    ...
    private function _readStream(\MailPoetVendor\Swift_OutputByteStream $os)
    {
        $string = '';
    ...
    private function _notifyEncoderChanged(\MailPoetVendor\Swift_Mime_ContentEncoder $encoder)
    {
        foreach ($this->_immediateChildren as $child) {
    ...
    {
        if ($this->_cache instanceof \MailPoetVendor\Swift_KeyCache) {
