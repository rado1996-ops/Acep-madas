...
abstract class Swift_Transport_AbstractSmtpTransport implements \MailPoetVendor\Swift_Transport
{
     ...
     */
    public function __construct(\MailPoetVendor\Swift_Transport_IoBuffer $buf, \MailPoetVendor\Swift_Events_EventDispatcher $dispatcher)
    {
     ...
     * @return int
     */
    public function send(\MailPoetVendor\Swift_Mime_Message $message, &$failedRecipients = null)
    {
        $sent = 0;
        ...
        if (!($reversePath = $this->_getReversePath($message))) {
            $this->_throwException(new \MailPoetVendor\Swift_TransportException('Cannot send message without a sender address'));
        }
    ...
    protected function _streamMessage(\MailPoetVendor\Swift_Mime_Message $message)
    {
    ...
    protected function _getReversePath(\MailPoetVendor\Swift_Mime_Message $message)
    {
    ...
    protected function _throwException(\MailPoetVendor\Swift_TransportException $e)
    {
    ...
    protected function _assertResponseCode($response, $wanted)
    {
        ...
        if (!$valid) {
            $this->_throwException(new \MailPoetVendor\Swift_TransportException('Expected response code ' . \implode('/', $wanted) . ' but got code ' . '"' . $code . '", with message "' . $response . '"', $code));
        }
    }
    ...
    protected function _getFullResponse($seq)
    {
        ...
        try {
            ...
            $this->_throwException(new \MailPoetVendor\Swift_TransportException($e->getMessage()));
        }
        return $response;
    }
    ...
    private function _sendTo(\MailPoetVendor\Swift_Mime_Message $message, $reversePath, array $to, array &$failedRecipients)
    {
        if (empty($to)) {
            return 0;
        }
    ...
    private function _sendBcc(\MailPoetVendor\Swift_Mime_Message $message, $reversePath, array $bcc, array &$failedRecipients)
    {
        $sent = 0;
