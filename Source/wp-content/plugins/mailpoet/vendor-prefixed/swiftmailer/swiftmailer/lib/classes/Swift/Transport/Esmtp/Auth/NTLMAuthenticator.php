...
class Swift_Transport_Esmtp_Auth_NTLMAuthenticator implements \MailPoetVendor\Swift_Transport_Esmtp_Authenticator
{
    ...
    protected function createDesKey($key)
    {
        ...
        foreach ($material as $k => $v) {
            ...
            $needsParity = (($this->uRShift($b, 7) ^ $this->uRShift($b, 6) ^ $this->uRShift($b, 5) ^ $this->uRShift($b, 4) ^ $this->uRShift($b, 3) ^ $this->uRShift($b, 2) ^ $this->uRShift($b, 1)) & 0x1) == 0;
