...
class Swift_Transport_StreamBuffer extends \MailPoetVendor\Swift_ByteStream_AbstractFilterableInputStream implements \MailPoetVendor\Swift_Transport_IoBuffer
{
    ...
    public function startTLS()
    {
        ...
        return \stream_socket_enable_crypto($this->_stream, \true, $cryptoType);
    }
    /**
    ...
    public function readLine($sequence)
    {
        if (isset($this->_out) && !\feof($this->_out)) {
            ...
            if (\strlen($line) == 0) {
                ...
                if ($metas['timed_out']) {
                    throw new \MailPoetVendor\Swift_IoException('Connection to ' . $this->_getReadConnectionDescription() . ' Timed Out');
                }
            }
            return $line;
        }
    ...
    private function _establishSocketConnection()
    {
        ...
        if (\false === $this->_stream) {
            throw new \MailPoetVendor\Swift_TransportException('Connection could not be established with host ' . $this->_params['host'] . ' [' . $errstr . ' #' . $errno . ']');
        }
