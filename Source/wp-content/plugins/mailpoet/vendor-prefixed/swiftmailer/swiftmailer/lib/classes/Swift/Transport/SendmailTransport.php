...
class Swift_Transport_SendmailTransport extends \MailPoetVendor\Swift_Transport_AbstractSmtpTransport
{
     ...
     * @var array
     */
    private $_params = array('timeout' => 30, 'blocking' => 1, 'command' => '/usr/sbin/sendmail -bs', 'type' => \MailPoetVendor\Swift_Transport_IoBuffer::TYPE_PROCESS);
    /**
    ...
    public function send(\MailPoetVendor\Swift_Mime_Message $message, &$failedRecipients = null)
    {
        ...
        if (\false !== \strpos($command, ' -t')) {
            ...
            $this->_throwException(new \MailPoetVendor\Swift_TransportException('Unsupported sendmail command flags [' . $command . ']. ' . 'Must be one of "-bs" or "-t" but can include additional flags.'));
        }
        return $count;
    }
