 ...
 */
class Swift_Transport_MailTransport implements \MailPoetVendor\Swift_Transport
{
     ...
     */
    public function __construct(\MailPoetVendor\Swift_Transport_MailInvoker $invoker, \MailPoetVendor\Swift_Events_EventDispatcher $eventDispatcher)
    {
        @\trigger_error(\sprintf('The %s class is deprecated since version 5.4.5 and will be removed in 6.0. Use the Sendmail or SMTP transport instead.', __CLASS__), \E_USER_DEPRECATED);
    ...
    public function send(\MailPoetVendor\Swift_Mime_Message $message, &$failedRecipients = null)
    {
        ...
        if (0 === $count) {
            $this->_throwException(new \MailPoetVendor\Swift_TransportException('Cannot send message without a recipient'));
        }
    ...
    private function _getReversePath(\MailPoetVendor\Swift_Mime_Message $message)
    {
