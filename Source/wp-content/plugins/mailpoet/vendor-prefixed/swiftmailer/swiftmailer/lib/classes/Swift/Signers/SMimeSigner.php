...
class Swift_Signers_SMimeSigner implements \MailPoetVendor\Swift_Signers_BodySigner
{
    ...
    public function signMessage(\MailPoetVendor\Swift_Message $message)
    {
        ...
        $message->setEncoder(\MailPoetVendor\Swift_DependencyContainer::getInstance()->lookup('mime.rawcontentencoder'));
     ...
     */
    protected function toSMimeByteStream(\MailPoetVendor\Swift_InputByteStream $inputStream, \MailPoetVendor\Swift_Message $message)
    {
        ...
        if (null !== $this->signCertificate && null !== $this->encryptCert) {
            $temporaryStream = new \MailPoetVendor\Swift_ByteStream_TemporaryFileByteStream();
     ...
     */
    protected function createMessage(\MailPoetVendor\Swift_Message $message)
    {
        $mimeEntity = new \MailPoetVendor\Swift_Message('', $message->getBody(), $message->getContentType(), $message->getCharset());
     ...
     */
    protected function messageStreamToSignedByteStream(\MailPoetVendor\Swift_FileStream $outputStream, \MailPoetVendor\Swift_InputByteStream $inputStream)
    {
     ...
     * @throws Swift_IoException
     */
    protected function messageStreamToEncryptedByteStream(\MailPoetVendor\Swift_FileStream $outputStream, \MailPoetVendor\Swift_InputByteStream $is)
    {
    ...
    protected function streamToMime(\MailPoetVendor\Swift_OutputByteStream $fromStream, \MailPoetVendor\Swift_Message $message)
    {
        ...
        if ('multipart/signed;' === \substr($headers['content-type'], 0, 17)) {
            if (!\preg_match('/boundary=("[^"]+"|(?:[^\\s]+|$))/is', $headers['content-type'], $contentTypeData)) {
                throw new \MailPoetVendor\Swift_SwiftException('Failed to find Boundary parameter');
            }
            $boundary = \trim($contentTypeData['1'], '"');
            ...
            if (null === $this->headerFactory) {
                $this->headerFactory = \MailPoetVendor\Swift_DependencyContainer::getInstance()->lookup('mime.headerfactory');
            }
            $message->setContentType($headers['content-type']);
