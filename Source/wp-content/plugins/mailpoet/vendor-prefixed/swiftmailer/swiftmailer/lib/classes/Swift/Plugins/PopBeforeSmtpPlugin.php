        ...
        if (isset($this->_connection)) {
            ...
            if (!isset($this->_socket)) {
                if (!($socket = \fsockopen($this->_getHostString(), $this->_port, $errno, $errstr, $this->_timeout))) {
                    throw new \MailPoetVendor\Swift_Plugins_Pop_Pop3Exception(\sprintf('Failed to connect to POP3 host [%s]: %s', $this->_host, $errstr));
                }
                $this->_socket = $socket;
    ...
    public function disconnect()
    {
        if (isset($this->_connection)) {
            ...
            if (!\fclose($this->_socket)) {
                throw new \MailPoetVendor\Swift_Plugins_Pop_Pop3Exception(\sprintf('POP3 host [%s] connection could not be stopped', $this->_host));
            }
            $this->_socket = null;
        }
    }
