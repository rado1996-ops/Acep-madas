...
class Swift_Transport_Esmtp_AuthHandler implements \MailPoetVendor\Swift_Transport_EsmtpHandler
{
    ...
    public function afterEhlo(\MailPoetVendor\Swift_Transport_SmtpAgent $agent)
    {
        if ($this->_username) {
            ...
            throw new \MailPoetVendor\Swift_TransportException('Failed to authenticate on SMTP server with username "' . $this->_username . '" using ' . $count . ' possible authenticators');
        }
    }
    /**
     * Not used.
     ...
     * Not used.
     */
    public function onCommand(\MailPoetVendor\Swift_Transport_SmtpAgent $agent, $command, $codes = array(), &$failedRecipients = null, &$stop = \false)
    {
    }
    /**
