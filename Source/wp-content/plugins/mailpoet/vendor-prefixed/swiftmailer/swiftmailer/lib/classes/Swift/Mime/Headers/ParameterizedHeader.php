 ...
 * @author Chris Corbyn
 */
class Swift_Mime_Headers_ParameterizedHeader extends \MailPoetVendor\Swift_Mime_Headers_UnstructuredHeader implements \MailPoetVendor\Swift_Mime_ParameterizedHeader
{
    /**
     ...
     */
    public function __construct($name, \MailPoetVendor\Swift_Mime_HeaderEncoder $encoder, \MailPoetVendor\Swift_Encoder $paramEncoder = null, \MailPoetVendor\Swift_Mime_Grammar $grammar)
    {
    ...
    private function _createParameter($name, $value)
    {
        ...
        if (\count($valueLines) > 1) {
            ...
            foreach ($valueLines as $i => $line) {
                $paramLines[] = $name . '*' . $i . $this->_getEndOfParameterValue($line, \true, $i == 0);
            }
            return \implode(";\r\n ", $paramLines);
        } else {
            return $name . $this->_getEndOfParameterValue($valueLines[0], $encoded, \true);
        }
    }
    /**
