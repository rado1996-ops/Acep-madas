 ...
 */
class Rule implements \MailPoetVendor\Sabberworm\CSS\Renderable, \MailPoetVendor\Sabberworm\CSS\Comment\Commentable
{
    private $sRule;
    private $mValue;
    ...
    public function setValues($aSpaceSeparatedValues)
    {
        ...
        if (\count($aSpaceSeparatedValues) > 1) {
            $oSpaceSeparatedList = new \MailPoetVendor\Sabberworm\CSS\Value\RuleValueList(' ', $this->iLineNo);
        }
        foreach ($aSpaceSeparatedValues as $aCommaSeparatedValues) {
            $oCommaSeparatedList = null;
            if (\count($aCommaSeparatedValues) > 1) {
                $oCommaSeparatedList = new \MailPoetVendor\Sabberworm\CSS\Value\RuleValueList(',', $this->iLineNo);
            }
    ...
    {
        if (!$this->mValue instanceof \MailPoetVendor\Sabberworm\CSS\Value\RuleValueList) {
    ...
    public function addValue($mValue, $sType = ' ')
    {
        ...
        if (!$this->mValue instanceof \MailPoetVendor\Sabberworm\CSS\Value\RuleValueList || $this->mValue->getListSeparator() !== $sType) {
            $mCurrentValue = $this->mValue;
            $this->mValue = new \MailPoetVendor\Sabberworm\CSS\Value\RuleValueList($sType, $this->iLineNo);
            if ($mCurrentValue) {
    ...
    public function render(\MailPoetVendor\Sabberworm\CSS\OutputFormat $oOutputFormat)
    {
        ...
        if ($this->mValue instanceof \MailPoetVendor\Sabberworm\CSS\Value\Value) {
