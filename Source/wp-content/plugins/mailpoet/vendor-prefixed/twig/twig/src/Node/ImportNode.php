 ...
 */
class ImportNode extends \MailPoetVendor\Twig\Node\Node
{
    public function __construct(\MailPoetVendor\Twig\Node\Expression\AbstractExpression $expr, \MailPoetVendor\Twig\Node\Expression\AbstractExpression $var, $lineno, $tag = null)
    {
    ...
    public function compile(\MailPoetVendor\Twig\Compiler $compiler)
    {
        ...
        if ($this->getNode('expr') instanceof \MailPoetVendor\Twig\Node\Expression\NameExpression && '_self' === $this->getNode('expr')->getAttribute('name')) {
            $compiler->raw('$this');
        } else {
