...
class NullCoalesceExpression extends \MailPoetVendor\Twig\Node\Expression\ConditionalExpression
{
    ...
    {
        $test = new \MailPoetVendor\Twig\Node\Expression\Test\DefinedTest(clone $left, 'defined', new \MailPoetVendor\Twig\Node\Node(), $left->getTemplateLine());
        // for "block()", we don't need the null test as the return value is always a string
        if (!$left instanceof \MailPoetVendor\Twig\Node\Expression\BlockReferenceExpression) {
            $test = new \MailPoetVendor\Twig\Node\Expression\Binary\AndBinary($test, new \MailPoetVendor\Twig\Node\Expression\Unary\NotUnary(new \MailPoetVendor\Twig\Node\Expression\Test\NullTest($left, 'null', new \MailPoetVendor\Twig\Node\Node(), $left->getTemplateLine()), $left->getTemplateLine()), $left->getTemplateLine());
        }
