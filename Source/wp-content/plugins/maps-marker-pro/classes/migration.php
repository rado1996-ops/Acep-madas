...
class Migration {
	...
	public function data_migration() {
		...
		$tools = Maps_Marker_Pro::get_instance('MMP\Menu_Tools');
		...
		for ($i = 1; $i <= $markers_batches; $i++) {
			...
			$db->add_markers($markers_converted, false);
		}
	...
	private function convert_settings($settings) {
		...
		if (isset($settings['geocoding_provider_fallback'])) {
			switch ($settings['geocoding_provider_fallback']) {
				case 'algolia-places':
					$converted['geocodingProviderFallback'] = 'algolia';
					break;
				case 'photon':
					$converted['geocodingProviderFallback'] = 'photon';
					break;
				case 'mapquest-geocoding':
					$converted['geocodingProviderFallback'] = 'mapquest';
					break;
				case 'google-geocoding':
					$converted['geocodingProviderFallback'] = 'google';
					break;
				default:
					break;
			}
	...
	private function convert_layer($layer, $settings) {
		...
		$converted['geojson'] = '{}';
		$converted['created_by'] = $layer->createdby;
		$converted['created_on'] = $layer->createdon;
		$converted['updated_by'] = $layer->updatedby;
	...
	private function convert_marker($marker) {
		...
		$converted['link'] = '';
		$converted['blank'] = '1';
		$converted['created_by'] = $marker->createdby;
		$converted['created_on'] = $marker->createdon;
		$converted['updated_by'] = $marker->updatedby;
	...
	public function copy_icons($settings) {
		if (($handle = @opendir(Maps_Marker_Pro::$icons_dir)) !== false) {
			while (($file = readdir($handle)) !== false) {
				if ($file !== '.' && $file !== '..') {
					unlink(Maps_Marker_Pro::$icons_dir . $file);
				}
			}
			closedir($handle);
		}
		...
		if (($handle = @opendir($icons_dir)) !== false) {
			while (($file = readdir($handle)) !== false) {
				...
				if (!is_dir($icons_dir . $file) && in_array($ext, $allowed)) {
					copy($icons_dir . $file, Maps_Marker_Pro::$icons_dir . $file);
				}
			}
			closedir($handle);
		}
