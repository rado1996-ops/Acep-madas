...
class DB {
	...
	public function get_map_shortcodes($id) {
		...
		$results = $wpdb->get_results($wpdb->prepare(
			...
			'%[' . $wpdb->esc_like(Maps_Marker_Pro::$settings['shortcode']) . '%map="' . $wpdb->esc_like($id) . '"%]%',
			'%[' . $wpdb->esc_like(Maps_Marker_Pro::$settings['shortcode']) . '%layer="' . $wpdb->esc_like($id) . '"%]%'
		));
	...
	public function add_maps($data) {
		...
		$sql = "INSERT INTO {$wpdb->prefix}mmp_maps ({$cols}) VALUES ";
		foreach ($data as $map) {
			$sql .= $wpdb->prepare("({$prep}),", array_values($map));
		}
	 ...
	 * @param bool $geocode (optional) Whether to get the marker coordinates via geocoding
	 * @param int $id ID for the new marker
	...
	public function add_markers($data, $geocode = false) {
		...
		$sql = "INSERT INTO {$wpdb->prefix}mmp_markers ({$cols}) VALUES ";
		foreach ($data as $marker) {
			$sql .= $wpdb->prepare("({$prep}),", array_values($marker));
		}
		$sql = substr($sql, 0, -1); // Remove trailing comma from loop-generated query
